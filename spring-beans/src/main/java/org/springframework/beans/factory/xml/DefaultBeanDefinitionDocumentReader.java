...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	/**
	 * 准备root参与注册
	 * 		得到doc的root,然后进入到真正注册BeanDefinition方法的doRegisterBeanDefinition中,
	 * 		doRegisterBeanDefinitions方法在后续解析的beans元素的时候也会再递归被调用,所以它是解析的入口
	 */
	...
	/**
	 * 1.处理profile属性
	 * 		profile是在beans节点配置的属性,可以同时配置多个,首先程序会判断beans节点是否
	 * 		定义了profile属性,如果配置了,再判断是否和环境中定义的一致,如果不一致,不会浪费性能
	 * 		去解析了,即至少要定义一个和环境中匹配的profile属性，断言环境environment不可能
	 * 		为空，利用profile可以定义多套配置，实现他们之间的切换，如最常见的数据库配置切换
	 * 2.前后置方法
	 * 		在真正解析之前和之后位置添加了preProcessXml和postProcessXml方法,为空实现,
	 * 		但因为是protected修饰,可以判断使用了模板方法设计模式,可以在子类中覆盖这两个方法来
	 * 		添加解析前后的处理动作,此处是使用模板方法中的钩子方法的案例
	 * 3.解析类delegate
	 * 		首先创建一个新的解析，解析完成后赋值会原来的解析类delegate
	 */
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
		//node.getNamespaceURI()值,如果为空或为http://www.springframework.org/schema/beans,
		//则采用当前类中默认的解析方式来解析元素,否则用定制解析元素的方法解析
