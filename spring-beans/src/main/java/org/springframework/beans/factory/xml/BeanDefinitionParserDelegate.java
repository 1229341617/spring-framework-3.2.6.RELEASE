...
public class BeanDefinitionParserDelegate {
	...
	/**
	 * 1.解析id和name-name可以取多个用,分隔开
	 * 		1):id != null && name != null
	 * 			id为bean的name,name为bean的alias
	 * 		2):id == null && name != null
	 * 			name生成的别名的第一个值会赋值给id且为bean的name,同时该bean的alias集合中此alias
	 * 		3):id != null && name == null
	 * 			bean只有id无别名,id为bean的name
	 * 		4):id == null && name == null
	 * 			会利用class属性生成id,规则为className#0为bean的name,而此时className为该bean的alias
	 * 2.进一步解析bean中的其他所有元素
	 */
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			//<Bean>元素中没有配置id、别名或者name
			if (!StringUtils.hasText(beanName)) {
				try {
					//<Bean>元素中没有包含子元素，则为解析的Bean生成一个唯一的beanName并注册
					if (containingBean != null) {
	...
	/**
	 * 1.创建BeanDefinition
	 * 		解析出parent、class属性,构造GenericBeanDefinition对象;
	 * 2.解析bean元素当级的其他属性
	 * 		如scope、init-method、destory-method等属性;
	 * 3.解析bean元素二级的属性
	 * 		1):七个属性
	 * 			元数据-meta、lookup-method、replaced-method
	 * 			构造方法参数-constructor-arg、property、qualifier
	 * 			description
	 * 		2):关键解析步骤详解
	 * 			1>:元数据的使用
	 * 				1]:配置
	 * 					<beans>
	 *						<bean id="myTestBean" class="_01_容器的基本实现.MyTestBean">
	 *							<meta key="a" value="hello-a"/>
	 *						</bean>
	 *					</beans>
	 * 				2]:代码
	 * 					XmlBeanFactory bf = new XmlBeanFactory(new ClassPathResource("applicationContext.xml"));
	 *					String metaValue = bf.getBeanDefinition("myTestBean").getAttribute("a"));//hello-a
	 */
	 ...
	 */
	/**
	 * 解析bean元素中除id、name、parent和class之外的属性
	 */
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
			BeanDefinition containingBean, AbstractBeanDefinition bd) {
		//解析scop属性
		if (ele.hasAttribute(SCOPE_ATTRIBUTE)) {
			//scope属性在spring的1.x版本的时候还没有出现,1.x版本时候升级为scope属性
			// Spring 2.x "scope" attribute
			bd.setScope(ele.getAttribute(SCOPE_ATTRIBUTE));
			//scop和singleton两个属性不能并存，不然spring报如下异常
		...
		//解析singleton属性
			...
			//嵌入的beandefinition没有指定scop属性的情况下，默认使用父beandefinition的属性值
	...
	//将meta中的key和value值封装到BeanMetadataAttribute中
	 ...
	 */
	/*<bean>
		<constructor-arg index="" type="" value="" ref="" name="">
		</constructor-arg>
	</bean>*/
	public void parseConstructorArgElement(Element ele, BeanDefinition bd) {
		//分别得到<constructor-arg>元素中的index、type、name属性值
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						//解析<constructor-arg>元素的子元素
						Object value = parsePropertyValue(ele, bd, null);
						//通过解析得到的value封装ConstructorArgumentValues，后续依次封装type和name
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, String propertyName) {
		...
		//获取<property>的所有子元素，只能是其中一种类型:ref,value,list,map，array,props等等
		NodeList nl = ele.getChildNodes();
		...
		if (hasRefAttribute) {
			...
			//一个指向运行时所依赖对象的引用ref=RuntimeBeanReference
		...
		//此处可以得出，如果property元素有子元素，则不需要有value或者ref属性的值
			...
			//propery属性中既无ref属性值，也无value属性值，也无子元素，则解析出错返回null
	...
	public Object parsePropertySubElement(Element ele, BeanDefinition bd, String defaultValueType) {
		//如果<property>没有使用Spring默认的命名空间，就不能使用默认的
		//解析规则解析了，此时需要使用用户自定义的规则解析内嵌元素
